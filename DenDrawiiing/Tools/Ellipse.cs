...
namespace DenDrawiiing.Tools
{
    public class Ellipse : Tool
    {
        ...
        public override void DrawStart(Point p, Graphics canvas, Color color, Bitmap img)
        {
            ...
            if (rsize.Width <= size || rsize.Height <= size)
                using (SolidBrush brush = new SolidBrush(color))
                    canvas.FillEllipse(brush, rsx - size / 2, rsy - size / 2, rsize.Width + size, rsize.Height + size);
            else
                canvas.DrawEllipse(pen, rsx, rsy, rsize.Width, rsize.Height);
            pen.Dispose();
        }
        ...
        public override void Serialize(Graphics bmp, Color color, Bitmap img)
        {
            ...
            if (rsize.Width <= size || rsize.Height <= size)
                using (SolidBrush brush = new SolidBrush(color))
                    bmp.FillEllipse(brush, rsx - size / 2, rsy - size / 2, rsize.Width + size, rsize.Height + size);
            else
                bmp.DrawEllipse(pen, rsx, rsy, rsize.Width, rsize.Height);
            pen.Dispose();
        }
    }
}
